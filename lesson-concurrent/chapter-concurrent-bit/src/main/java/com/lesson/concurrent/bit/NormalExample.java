...
public class NormalExample {
	...
	public static String lengthString(int length, String fill) {
		String[] results = new String[length];
		Arrays.fill(results, 0, length, fill);
		return StringUtils.join(results, StringUtils.EMPTY);
	}

	public static String toBinaryString(int i) {
		String result = Integer.toBinaryString(i);
		String[] results = new String[32];
		String[] src = result.split(StringUtils.EMPTY);

		System.arraycopy(src, 0, results, 32 - src.length, src.length);

		Arrays.fill(results, 0, Math.max(32 - src.length, 1), "0");
...
//        log.info("{}",((p&~4)==4));
	...
	public static void example3() {
		...
		System.out.println(toBinaryString(-16 >>> 1));


		System.out.println(toBinaryString(Integer.MAX_VALUE));
		System.out.println(Integer.valueOf(lengthString(32, "1"), 2));
